...
std::unique_ptr<CABitmapData> CAFreeTypeFont::initWithString(const std::string& pText, CAFont& font, int inWidth, int inHeight,
	CATextAlignment hAlignment, CAVerticalTextAlignment vAlignment, std::vector<TextViewLineInfo>* pLinesText)
{
	...
	CATempTypeFont::getInstance().initTempTypeFont(font.fontSize);

_AgaginInitGlyphs:
	m_inWidth = inWidth;
	m_inHeight = inHeight;
	m_lineSpacing = font.lineSpacing;
	m_bWordWrap = font.wordWrap;
	m_bBold = font.bold;
	m_bItalics = font.italics;
	m_bUnderLine = font.underLine;
	m_bDeleteLine = font.deleteLine;
	m_cFontColor = font.color;
	...
	return std::make_unique<CABitmapData>(pData, width * height * 4, width, height);
}
...
static unsigned char* getFileData(const std::string& filename, const char* mode, unsigned long* size)
{
...
unsigned char* CAFreeTypeFont::loadFont(const std::string& pFontName, unsigned long *size, int& ttfIndex)
{
    ...
    {
        pBuffer = getFileData(path.c_str(), "rb", size);
        if (pBuffer == NULL)
        {
            for (unsigned int i = 6; i < path.length(); ++i)
            {
    ...
    if (pBuffer == NULL)
    {
        ...
        pBuffer = getFileData(fontName, "rb", size);
        ttfIndex = 0;
    }
	...
	if (pBuffer == NULL)
	{
        ...
        pBuffer = getFileData(pFontName, "rb", size);
